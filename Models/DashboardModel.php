	...
	class DashboardModel extends Pgsql
	{
		...
		public function selectPagosMes(int $anio, int $mes){
			...
			$sql = "SELECT p.tipopagoid, tp.tipopago, COUNT(p.tipopagoid) as cantidad, SUM(p.monto) as total 
					...
					WHERE (EXTRACT (MONTH FROM p.fecha)) = $mes AND (EXTRACT(YEAR FROM p.fecha)) = $anio GROUP BY p.tipopagoid,tp.tipopago";
			$pagos = $this->selec_all($sql);
			$meses = Meses();
		...
		public function selectVentasMes(int $anio, int $mes){
			...
			for ($i=0; $i < $dias ; $i++) { 
				...
				$sql = "SELECT EXTRACT (DAY from fecha) AS dia, COUNT(idpedido) AS cantidad, SUM(monto) AS total 
						FROM pedido 
						WHERE DATE(fecha) = '$fechaVenta' AND status = 'Completo' ".$where."group by EXTRACT (DAY from fecha)";
				$ventaDia = $this->select($sql);
				$ventaDia['dia'] = $n_dia;
				if(!empty($ventaDia['total'])){
		...
		public function selectVentasAnio(int $anio){
			...
			for ($i=1; $i <= 12; $i++) { 
				...
				$sql = "SELECT $anio AS anio, $i AS mes, SUM(monto) AS venta 
						FROM pedido 
						WHERE (EXTRACT (MONTH FROM fecha))= $i AND (EXTRACT(YEAR FROM fecha)) = $anio AND status = 'Completo' 
		...
		public function productosTen(){
			$sql = "SELECT * FROM producto WHERE status = 1 ORDER BY idproducto DESC LIMIT 5 ";
			$request = $this->selec_all($sql);
			return $request;
		}
	}
